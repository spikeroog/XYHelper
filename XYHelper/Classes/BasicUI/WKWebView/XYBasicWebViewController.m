...
typedef NS_ENUM(NSInteger, WkWebViewShowType) {
    WkWebViewShowTypeWebPage = 0, // h5网页
    WkWebViewShowTypeLocalFile // 本地h5文件
};

@interface XYBasicWebViewController ()
...
@property (nonatomic, assign) WkWebViewShowType type;

@end

@implementation XYBasicWebViewController
...
- (void)localHtmlWithUrl:(NSString *)urlString {
    self.type = WkWebViewShowTypeWebPage;
...
- (void)localHtmlWithLocalFile:(NSString *)localFile {
    self.type = WkWebViewShowTypeLocalFile;
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if ([keyPath isEqualToString:NSStringFromSelector(@selector(estimatedProgress))]
        && object == _webView) {
        kLog(@"网页加载进度 = %f",_webView.estimatedProgress);
...
- (WKWebView *)webView {
    if (_webView == nil) {
        ...
        if (self.type == WkWebViewShowTypeWebPage) {
            // 加载js网页
            [_webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:self.urlString]]];
        } else if (self.type == WkWebViewShowTypeLocalFile) {
            // 加载本地js文件
            NSString *path = [[NSBundle mainBundle] pathForResource:self.localFile ofType:nil];
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation {
    ...
    if([urlstring isEqualToString:@"http://m.toutiao.com/profile/52672332919/#mid=52672332919"]){
        //执行JS
        [webView evaluateJavaScript:@"var openToDay=document.getElementsByTagName(\"div\")[0];openToDay.parentNode.removeChild(openToDay);document.getElementById(\"userhead\").style.display=\"none\";document.getElementById(\"profilepanel\").style.display=\"none\";document.getElementById(\"navlist\").childNodes[3].click();"
